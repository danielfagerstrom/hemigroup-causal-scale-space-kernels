...
| 11 The signaling form | `lem:mellin-data`, `lem:mellin-vertical`, `lem:inversion-symbol`, `lem:symbol-rigidity`, `def:inversion-operator`, `lem:inversion-operator-action`, `lem:profile-eigenfunction`, `lem:symbol-uniqueness`, `lem:delayed-average-mellin` — Theorem 4$'$'s Mellin data, the symbol `B`, the inversion operator, its eigenfunction relation `A[H(s·)] = s·H(s·)` (Theorem 4$'$(1)), the uniqueness that earns the definite article in *the* inversion (Theorem 4$'$(3)), `lem:signaling-mellin-form`, `lem:fractional-integral-derivative`, `lem:memory-fractional-integrals`, `thm:signaling-form` — Theorem 4$'$'s Mellin data, the symbol `B`, the inversion operator, its eigenfunction relation (Theorem 4$'$(1)), the uniqueness that earns the definite article in *the* inversion (Theorem 4$'$(3)), and Theorem 4$'$(2)'s Mellin form up to its derivative clause; `lem:standing-kernel-readings` (11.21, added 2026-08-29) — the two readings `def:standing-hypothesis`'s clauses used to assert as unchecked parentheticals, now proved (its siblings `lem:standing-levy-reading`, 11.22, stated in the `(b_0,k)` data of (7.1) rather than in `T_1`'s law, and `lem:zstar-log-growth`, 11.23, the log-growth rate `z_* = lim F(s)/log s` that reading omits, are both `\notready`; see `Formalization/Skeleton/Chapter11.lean`). **The chapter now has a well-posedness ending** (11.24–11.27, added 2026-08-29): `def:mellin-solution` names what a solution of the signaling problem *is* (Theorem 4$'$(2)'s informal gloss, formalised, not restated); `lem:mode-rigidity` kills the period-one ambiguity the eigenvalue recursion has — the reading Theorem 4$'$(3)'s operator-uniqueness does not settle — under a normalisation the field satisfies for free, and is `\notready` with a sorry-marked target type in `Formalization/Skeleton/Chapter11.lean` (priced, not attempted: the missing piece is a periodic-extension construction Mathlib does not carry); `cor:signaling-wellposed` assembles existence and *solution*-uniqueness, reusing ledger A12 for a second, cheaper purpose (`blueprint/AXIOMS.md`); and `cor:signaling-classical` (split off on the Lemma 7.1 precedent) upgrades the Mellin-level identity to a genuine pointwise PDE under a decay hypothesis satisfied by the Gamma, stable and Bessel-K families and correctly failed by the pure delay; and `cor:signaling-hadamard` (added 2026-09-01) assembles Hadamard's three demands — existence and uniqueness restated, the data contraction from the representation, and stability under pointwise perturbation of the exponent via `prop:laplace-continuity` — so that the chapter's well-posedness claim is a stated and proved corollary rather than arc prose. None of the five carry a `\leanok` proof. |
...
them* is now **empty**. Everything else:
