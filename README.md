...
| 10 The scale-Cauchy problem | `lem:delay-core` — the core `𝒟`, dense in `X₀` and invariant under the delay semigroup and under every `Φ`, with the `L¹` difference quotient and the two-sided delay estimate; and `def:phillips-generator` — the per-scale generator `φ_x(∂_t)f = b₀f' + ∫(f - T_rf)ν_x(dr)`, an `X₀`-valued Bochner integral that needed no theory the development did not have |
...
`lem:potential-kernel-scaling`, and chapter 10's `lem:generator-properties`, whose five clauses
are stated and open. Everything else:
