...
Everything §§4–6 contributes to Theorem 7.3 is verified with it: the convolution representation, the additivity and infinite divisibility of the exponents (Theorem 5.2), and the similarity form forced by covariance. In §9, the memory kernel, its transform, the Sonine conservation identity $\kappa^{(x)} * \ell^{(x)} = \mathrm{Leb}$ and the existence and uniqueness of the potential kernel are verified. Of §11, all fourteen numbered items are. Of §12, the log-convexity of the moment function (Lemma 12.4) and the vanishing $B(0) = 0$ (Lemma 12.3(1)) are.
...
§12 is a mixed case and worth separating out, because the obstruction there is narrower than it first appears. Two of its lemmas are verified, and neither needed anything Mathlib lacks. What is not verified divides in three. Lemma 12.2 needs an operator to be compared with a differential expression on $C_c^\infty$, which is available but laborious. The two classification results the theorem turns on — that a local operator satisfying the positive maximum principle has order at most two [Courrège], and that the recursion $m(z{+}1) = Q(z)m(z)$ has a unique log-convex solution [Krull–Webster] — are cited interfaces, and the second has a curious status: in the order-2 case it is the classical Bohr–Mollerup theorem, which Mathlib does carry, so that half could in principle be discharged at the cost of extending $m$ below the origin through the recursion; the order-$n$ ladder of Proposition 12.6 needs the general theorem and cannot. Only the Bessel identifications need $K_\nu$, which Mathlib does not have. So the honest statement is that §12's gap is two citations and one laborious lemma, not an absent theory.
