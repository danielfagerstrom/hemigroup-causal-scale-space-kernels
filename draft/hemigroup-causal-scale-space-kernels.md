...
The central results of this paper have been formalized in the Lean 4 proof assistant, on top of its mathematical library Mathlib, and this subsection says exactly which, resting on exactly what. It is self-contained: nothing below depends on reading the formal development.

**What is proved.** Both main theorems are machine-checked in full.

- **Theorem 7.3**, the characterization: a family $(\Phi_{x,y})$ satisfies (A1)–(A8) and (ND) if and only if it is convolution by the kernels of an exponent of the form (7.1), read through a gauge — together with the uniqueness of the pair (gauge, exponent). All three parts are verified: the construction, the analysis, and the uniqueness.
- **Theorem 11.6**, the signaling form (Theorem 4′): the profiles $H(s\,\cdot)$ are eigenfunctions of the inversion operator $A$ with eigenvalue $s$; the field solves the signaling problem, in its Laplace and its Mellin form; and $A$ is the unique inversion in the covariant Mellin class.

Everything §§4–6 contributes to Theorem 7.3 is verified with it: the convolution representation, the additivity and infinite divisibility of the exponents (Theorem 5.2), and the similarity form forced by covariance. In §9, the memory kernel, its transform, the Sonine conservation identity $\kappa^{(x)} * \ell^{(x)} = \mathrm{Leb}$ and the existence and uniqueness of the potential kernel are verified. Of §11, all fourteen numbered items are. Of §12, the log-convexity of the moment function (Lemma 12.4), the vanishing $B(0) = 0$ (Lemma 12.3(1)) and the uniqueness of the log-convex solution of the moment recursion in the order-2 case are.

**What it rests on.** Two analytic facts are taken from the literature rather than reproved. They are the *only* things the verified results assume beyond the logical foundations of the proof assistant itself, and each is cited with a page reference checked against the source.

1. *Existence of a subordinator from its triple.* Given a drift $b_0 \ge 0$ and a Lévy measure $\nu$ with $\int (1 \wedge t)\,\nu(dt) < \infty$, there is a probability measure on $[0,\infty)$ whose Laplace transform is $\exp[-(b_0 s + \int (1 - e^{-st})\nu(dt))]$ [SSV, Thm. 5.2, p. 49]. This is what turns an admissible exponent into actual kernels, and it is the only thing the constructive direction of Theorem 7.3 needs that is not proved here.
2. *Self-decomposability, in the analysis direction.* If every dilation increment $F(b\,\cdot) - F(a\,\cdot)$ of an exponent is again an exponent, then its Lévy measure has a nonincreasing density $k(t)/t$ [SSV, Prop. 5.17, p. 57]. This is the implication (1) ⇒ (3) of Lemma 7.1, and it is the only thing the analysis direction needs that is not proved here.

The two are used in *different* halves and neither borrows the other's: the construction and the uniqueness of Theorem 7.3 use only (1); the analysis direction uses only (2). That separation is checked mechanically rather than claimed. The first is expected to be temporary — it is a construction, and carrying it out would make it a lemma; the second is not, for the reason in the next paragraph.

**Why these two and not more.** The classical route to these results passes through the theory of completely monotone and Bernstein functions — the Bernstein–Widder theorem, closure of $\mathrm{BF}$ under pointwise limits, uniqueness of the Lévy–Khintchine triple, composition rules. Formalizing led us to replace five arguments by shorter ones that use the *representation* (7.1) directly, and those five are now the proofs printed in this paper: Theorem 5.2 extracts the limiting triple from a weak limit instead of invoking closure; Lemma 9.4 constructs the potential kernel as a subordinator's potential measure instead of representing it by Bernstein–Widder; and three proofs in §11 lose an appeal to Stirling's formula, to the semigroup property of the Riemann–Liouville family, and to Mellin uniqueness respectively. What survives is exactly the place where the derivative-sign definition is irreducible: $B(s) = sF'(s)$ in Lemma 7.1 has no meaning until the representation is in hand, which is why fact (2) above is expected to be permanent.

**What is not formalized, and why.** §10 (Theorem 3′, the scale-Cauchy problem) is not, for a reason external to this paper: it needs the theory of $C_0$-semigroups and closed operators, which Mathlib does not yet carry. Two results in §9 — the distributional scale-evolution identity (Proposition 9.2) and the exact-inversion corollary — need distributions paired with measures, likewise absent. Proposition 9.7(3), the characterization of when $\kappa^{(x)}$ has a completely monotone density, is a statement *about* the CM class, which the formal development deliberately does not define; it is cited, not verified. §§8 and 13–14 are examples and implementation, formalized only where they were cheap.

§12 is a mixed case and worth separating out, because the obstruction there is narrower than it first appears. Three of its lemmas are verified — the log-convexity of the moment function, the vanishing $B(0) = 0$, and the uniqueness of the log-convex solution of the moment recursion in the order-2 case — and none of them needed anything Mathlib lacks. That last one deserves a word, because it removes a citation the theorem was stated to rest on: Theorem 12.5 applies the recursion $m(z{+}1) = Q(z)m(z)$ only where $Q$ is linear, and there the Krull–Webster uniqueness theorem is the classical Bohr–Mollerup theorem, which is available; the order-$n$ ladder of Proposition 12.6 has $n-1$ Gamma factors and does still need the general result. What remains unverified in §12 is therefore Lemma 12.2, where an operator must be compared with a differential expression on $C_c^\infty$ — available but laborious; Courrège's classification, that a local operator satisfying the positive maximum principle has order at most two, which is cited; and the Bessel identifications, which need $K_\nu$. So §12's gap is one citation, one laborious lemma, and one special function — not an absent theory.

**Where to look.** The development, its axiom ledger with page-verified citations for every cited interface, and a dependency graph in which each statement is coloured by its verification status, are available at the repository accompanying this paper. The ledger records eighteen interfaces the paper cites; the two above are the only ones the verified results actually consume.
...
*Which vocabulary does the work.* Proposition 2.3 is used as a bridge and not as a toolkit. The object the arguments below manipulate is the *representation* (3) — a drift and a Lévy measure — and almost every proof in §§4–11 works with it directly: the increments of Theorem 5.2 are produced as representations, not as pointwise limits of Bernstein functions; the potential kernel of Lemma 9.4 is constructed as a subordinator's potential measure, not represented from complete monotonicity; the Mellin calculus of §11 never mentions either class. Complete monotonicity and the derivative-sign definition of $\mathrm{BF}$ are genuinely needed at exactly three places, and it is worth naming them: Lemma 7.1's implications (1) ⇒ (2) ⇒ (3), where $B(s) = sF'(s)$ has no meaning until the representation is in hand; Proposition 9.7(3) and Proposition 13.6, where the CM class is the *subject* rather than a tool; and Proposition 2.3 itself, which identifies the two vocabularies once so that no later proof has to. A reader who wants the shortest path to Theorem 7.3 or Theorem 11.6 can take (3) as the definition of admissibility and never invoke Bernstein–Widder.
...
which is a Bernstein function of $s$ vanishing at $0{+}$ (Lévy–Khintchine form with finite Lévy measure $\Pi_n$).

It remains to pass to the limit, and the point is to extract the *representation* rather than to invoke closure of $\mathrm{BF}$ under pointwise limits. The total masses $\Pi_n([0,\infty)) = n$ diverge, because mass piles up at the origin — and that pile *is* the drift. So weight the measures by $1 - e^{-t}$, which vanishes at the origin at exactly the compensating rate, and substitute $v = 1 - e^{-t}$, carrying $[0,\infty]$ onto the compact $[0,1]$ with $\infty \mapsto 1$. The weighted measures then have uniformly bounded mass on a fixed compact, so a subsequence converges weakly and no tightness argument is needed. Integrating against $k_s(v) = \big(1 - (1-v)^s\big)/v$, which has a removable singularity at $v = 0$ with value $s$, gives

$$g_{x,y}(s) \;=\; b_0(x,y)\,s + \int_0^\infty \big(1 - e^{-st}\big)\,\nu_{x,y}(dt),$$

the mass the limit puts at $v = 0$ being the drift $b_0(x,y)$ — that is what the value $s$ at the removable singularity records — and the mass at $v = 1$, which would be a killing term, excluded by $g_{x,y}(0{+}) = 0$ (Lemma 4.2). So $g_{x,y} \in \mathrm{BF}_0$ by the representation itself, and Proposition 2.3(4) is not needed. The case $x = y$ is $g_{x,x} = 0 \in \mathrm{BF}_0$. ∎
...
*Proof.* With $B(s) := sF'(s) \in \mathrm{BF} \setminus \{0\}$ (Lemma 7.1(2)), $\varphi_x(s) = B(xs)/x \in \mathrm{BF} \setminus \{0\}$, and $\varphi_x > 0$ on $(0,\infty)$ by Lemma 2.4.

For existence we *construct* the measure rather than representing it. Since $\varphi_x$ is a Bernstein function without killing term it is the exponent of a subordinator; let $(\mu^{(x)}_\tau)_{\tau \ge 0}$ be its laws, $\widehat{\mu^{(x)}_\tau}(s) = e^{-\tau \varphi_x(s)}$, and set

$$\ell^{(x)} \;:=\; \int_0^\infty \mu^{(x)}_\tau \, d\tau,$$

the subordinator's *potential measure*. By Tonelli its transform is $\int_0^\infty e^{-\tau \varphi_x(s)}\,d\tau = 1/\varphi_x(s)$ for every $s > 0$, which is finite, so $\ell^{(x)}$ is locally finite; uniqueness is uniqueness of the Laplace transform for locally finite measures whose transform converges somewhere (Lemma 2.5). Two remarks. This route uses no property of the CM class — neither $\mathrm{CM} \circ \mathrm{BF} \subseteq \mathrm{CM}$ nor Bernstein–Widder in its general form, both of which the obvious argument spends — at the cost only of the existence of the subordinator, which Theorem 7.3 has already used. And $\int_0^\infty \mu^{(x)}_\tau \,d\tau$ is a measure only because $\tau \mapsto \mu^{(x)}_\tau$ is measurable, which is where the increasing paths are actually used: $\mu^{(x)}_{\tau + \tau'} = \mu^{(x)}_\tau * \mu^{(x)}_{\tau'}$ together with causality makes $\tau \mapsto \mu^{(x)}_\tau\big((-\infty, r]\big)$ nonincreasing, hence measurable, and a Dynkin argument on $\{(-\infty, r]\}$ extends this to every Borel set.

The scaling law follows by comparing transforms: $\widehat{\ell^{(x)}}(s) = x/B(xs) = x\,\widehat{\ell^{(1)}}(xs)$. ∎
...
**Definition 11.1 (the standing hypothesis (H)).** An admissible exponent $F$ satisfies the *standing hypothesis* (H) if $F(\infty) = \infty$ and

$$z_* \;:=\; \sup\big\{\, \zeta > 0 :\ \mathbb{E}\big[T_1^{-\zeta}\big] < \infty \,\big\} \;>\; 1 .$$

Both clauses are conditions at the zero-delay end of the kernels. The first says exactly that they put no atom at zero delay — equivalently, $b_0 > 0$ or $\int_0^\infty k(t)\,t^{-1}dt = \infty$ — and it is automatic for every admissible $F \not\equiv 0$, so all of (H)'s bite is in the second; it is stated nonetheless, being what the proofs consume. The second implies $\int_0^\infty e^{-F(s)}\,ds = \mathbb{E}[T_1^{-1}] < \infty$, and the converse fails only at the endpoint: finiteness forces $z_* \ge 1$, not $z_* > 1$, and the endpoint is genuinely reached — the Gamma family at $\gamma = 1$ has $z_* = 1$ with $\mathbb{E}[T_1^{-1}] = \infty$. The second clause holds with $z_* = \infty$ whenever $b_0 > 0$, for the stable family, and for the Bessel-K family of Example 8.3; for the Gamma family $z_* = \gamma$, so (H) requires $\gamma > 1$ there. The rate is general: $z_* = \lim_{s\to\infty} F(s)/\log s$, which is $k(0^+)$ when $b_0 = 0$ and $\infty$ otherwise, so (H) reads: drift, or catalogue height above one at the origin. (The construction extends below $z_* = 1$ by meromorphic continuation; we do not pursue this.) The threshold $1$ is the one this section keeps meeting — see the proofs of Lemma 11.5 and Theorem 11.6(2), where it arises three separate times — and it is where the cone's extreme rays sit: every extreme ray with $b_0 = 0$ has $F(s) \sim \log s$, hence $z_* = 1$ (Remark 7.6), so those rays lie exactly *on* the boundary of (H). The correspondence is not exact in either direction — the pure delay is an extreme ray and satisfies (H) outright, having $b_0 > 0$, and the Gamma family at $\gamma \le 1$ fails (H) without being extreme — but the coincidence of the obligations at the extreme rays' abscissa marks the boundary of (H) as characteristic of the theory rather than of any one argument.
...
**Lemma 11.4 (uniqueness of the symbol).** If two operators of the form $x^{-1}B_1(\theta)$, $x^{-1}B_2(\theta)$ both satisfy $A[H(s\,\cdot)] = s\,H(s\,\cdot)$ — for a single $s > 0$, at every height $c \in (0, z_*-1)$; see the proof — then $B_1 = B_2$ on the strip *as meromorphic functions*: they agree on a punctured neighbourhood of every point of it, hence pointwise at every point where both are continuous, in particular at any point that is a pole of neither. Hence $A$ is the unique inversion within the covariant Mellin class.
...
at every point of the line $\operatorname{Re} z = c$ where the realising identity holds. Cancelling the nonvanishing factor $s^{-z}$ leaves $B_1(-z)\tilde H(z) = B_2(-z)\tilde H(z)$ there, and at a zero of $\tilde H$ both sides vanish, so the identity holds on the whole line; as $c$ ranges over $(0, z_*-1)$ the lines fill the strip. (This is where the hypothesis *at every height* is spent: with the identity on a single line one would need $B_1, B_2$ meromorphic on the strip and the identity theorem to reach the rest of it, and neither is assumed.) The zeros of $\tilde H$ are isolated (Lemma 11.2), so $\tilde H$ may be cancelled on a punctured neighbourhood of *every* point of the strip, the zeros included, which is the conclusion. Two economies are worth recording, since both tools are natural to reach for and neither is needed: no injectivity of the inverse Mellin transform, the shared realising function carrying the information that injectivity would otherwise have to recover; and a single dilation, the dilate contributing only the factor $s^{-z}$, which never vanishes. ∎
...
absolutely convergent. If moreover $f \in \mathcal{D}$, the same holds for $\partial_t u$ — the $X_0$-derivative of §10, equivalently: $u(\cdot, x)$ is the primitive of $\mathbb{E}[f'(\cdot - xT_1)]$ — with $I^z f$ replaced by $I^z f' = I^{z-1} f$; and in that case the identity extends to the wider strip $0 < \operatorname{Re} z < z_*$, since $f \in \mathcal{D}$ is bounded ($f(0) = 0$ and $f' \in X_0$ give $\|f\|_\infty \le \|f'\|_1$) and the weight $y^{z-1}$ is then integrable at the origin on its own. **Theorem 11.6(2) uses the wider strip**: it applies this lemma at $z - 1$, which $\operatorname{Re} z > 1$ places outside the range stated for $f \in X_0$. Of the three conditions defining $\mathcal{D}$, the proofs below use exactly three consequences and no others: boundedness ($f(0) = 0$ with $f' \in X_0$ gives $\|f\|_\infty \le \|f'\|_1$), which is what widens the strip; that $f$ is the primitive of $f'$, which is what the derivative clause asserts; and $f \in X_0$ itself, which is needed for the Laplace form of Theorem 11.6(2) and does *not* follow from the other two, a primitive of an $L^1$ function lying in $L^1$ only when $\int_0^\infty f' = 0$. The density of $\mathcal{D}$ and its invariance under $T_r$ and $\Phi_{x,y}$ — the substance of Lemma 10.1 — are not used here.
...
which is the claim by Lemma 11.2. For $f \in \mathcal{D}$: $u(\cdot,x) = \mu_{0,x} * f$ and $f = \mathbf{1}_{[0,\infty)} * f'$, so $u(\cdot,x) = \mathbf{1}_{[0,\infty)} * (\mu_{0,x} * f')$ — that is, $u(\cdot,x)$ is the primitive of $\mathbb{E}[f'(\cdot - xT_1)]$, which is what $\partial_t u = \mathbb{E}[f'(\cdot - xT_1)]$ asserts in $X_0$. (It is *not* an assertion about pointwise derivatives, and would be false as one: $f'$ exists only a.e., and $\mu_{0,x} * f'$ is an $L^1$ function with no pointwise values to be a derivative at.) Finally $I^z f' = I^{z-1} f$: substituting $f(r) = \int_0^r f'$ and exchanging the order of integration over $0 < \rho \le r \le t$ leaves $\int_\rho^t (t-r)^{z-2}dr = (t-\rho)^{z-1}/(z-1)$, and $\Gamma(z) = (z-1)\Gamma(z-1)$; the semigroup identity $I^{z-1}I^1 = I^z$ is not needed. ∎
...
**Remark 11.8 (classical interpretation; poles).** The time-domain equation in Theorem 11.6(2) is stated at the level of Mellin transforms; this is the honest general formulation, since $B$ may have poles (at zeros of $\tilde H$) and polynomial growth along vertical lines, so that $A u(t, \cdot)$ need not be given by an absolutely convergent integral for rough $f$ — note that on the field the identity is nevertheless regular, the factor $\tilde H(z-1)$ cancelling every denominator. That last statement is about the product as a *meromorphic function*; read pointwise it needs $\tilde H(z-1) \ne 0$, which holds off an isolated set (Lemma 11.2), and Theorem 11.6(2) is stated accordingly. The distinction recurs through this section — the same one that makes Lemma 11.4's conclusion an equality of meromorphic functions rather than of functions — and is worth keeping in view: a cancellation performed on symbols has to be justified at each point where the identity is used. When the symbol $B$ is a *polynomial*, $A$ is a local differential operator and the equation is a classical PDE; that is precisely the situation of Theorem 5′ (§12) and of Example 11.11 below.

Theorem 11.6(2) closes with an informal gloss — "$\partial_t u = A\,u$ read through the Mellin transform in $x$" — and clause (3) proves uniqueness of the *operator* $A$, not of the *solution* $u$: two different claims the single word "unique" can hide, and the gap is real — the eigenvalue recursion $s\,\tilde g(z) = B(1-z)\tilde g(z-1)$ has the classical period-one ambiguity, its general solution being $s^{-z}\tilde H(z)\,p(z)$ for arbitrary $1$-periodic $p$, the same obstruction Bohr–Mollerup closes for $\Gamma$. The blueprint's appended nodes 11.24–11.27 close it here: a definition of *Mellin solution of the signaling problem* (mirroring Theorem 11.6(2)'s own clauses, so the gloss has a referent); a rigidity lemma killing the periodic ambiguity under a normalization the field satisfies for free — its modes are dominated by the profile, uniformly over one period, the theory's counterpart of Bohr–Mollerup's log-convexity; the corollary that then earns "well posed", assembling existence with *solution*-uniqueness; and a classical-solution corollary — $\partial_t u(t,x) = (A u(t,\cdot))(x)$ pointwise — under vertical-line decay of $m(z) = \mathbb{E}[T_1^{-z}]$, which Gamma, stable ($\alpha < 1$) and Bessel-K satisfy while the pure delay fails it, correctly: that family's operator is local, Theorem 5′'s territory.
...
**Definition 12.1 (locality; positive maximum principle).** The inversion operator $A$ is *local (of order $n$)* if it agrees with a differential expression $\sum_{j=0}^{n} c_j(x)\,\partial_x^j$, $c_j \in C((0,\infty))$, $c_n \not\equiv 0$, on $C_c^\infty((0,\infty))$ *and on the profiles $H(s\,\cdot)$, $s > 0$*. (The profiles are in the test class because Lemma 12.2's (⇒) direction reads the symbol off them: $C_c^\infty((0,\infty))$ is disjoint from the only class on which $B$ is controlled, and passing from one to the other would be an approximation argument this section does not make. The maximum principle stays on $C_c^\infty((0,\infty))$, where a profile — with no interior maximum — would say nothing.) It satisfies the *positive maximum principle* (PMP) if $(Ag)(x_0) \le 0$ whenever $g \in C_c^\infty((0,\infty))$ attains a nonnegative maximum at $x_0$.
...
*Proof.* ($\Leftarrow$) $x^{j}\partial_x^{j} = \theta(\theta-1)\cdots(\theta - j + 1)$, so a polynomial $B$, expanded in falling factorials, gives $x^{-1}B(\theta) = \sum \gamma_j x^{j-1}\partial_x^j$, a differential expression of order $\deg B$. ($\Rightarrow$) Every operator of Definition 11.3 satisfies the covariance $A\Delta_\sigma = \sigma^{-1}\Delta_\sigma A$, $(\Delta_\sigma g)(x) = g(x/\sigma)$. Imposing this on $\sum c_j(x)\partial_x^j$ and comparing coefficients of $g^{(j)}(x/\sigma)$ gives $c_j(x)\sigma^{-j} = \sigma^{-1}c_j(x/\sigma)$, i.e. $c_j(x) = c_j(1)x^{j-1}$. Rewriting via falling factorials, $A$ agrees with $x^{-1}P(\theta)$ for the polynomial $P(w) = \sum_j \gamma_j E_j(w)$, $E_j(w) = \prod_{i<j}(-w-i)$. It remains to identify $P$ with $B$, and the comparison is made on the profiles — the class Definition 12.1 includes for this purpose, and the only one on which $B$ is controlled: for $g = H(s\,\cdot)$, locality gives $(Ag)(x) = \sum_j \gamma_j x^{j-1}\partial_x^j H(sx)$ and Theorem 11.6(1) gives $(Ag)(x) = sH(sx)$; the right side of the first, with $\partial_x^j H(sx) = (-s)^j\int t^j e^{-sxt}\mu(dt)$ and Lemma 11.2 applied to $t^j\mu(dt)$, has Mellin transform $P(w)\,\widetilde{H(s\cdot)}(w)$ on $0 < \operatorname{Re} w < z_*$, so $P$ realises the action of a symbol on $H(s\,\cdot)$ at every height, and Lemma 11.4 gives $B = P$ as meromorphic functions on the strip. ∎
...
One methodological remark, since the formalization changed the paper rather than merely certifying it. Five of the proofs printed above are not the ones first written. In each case the original argument invoked a named theorem about a *class* of functions — Bernstein–Widder, closure of $\mathrm{BF}$ under pointwise limits, the semigroup property of the Riemann–Liouville family, the asymptotics of $\Gamma$ on vertical lines, uniqueness of the Mellin transform — where an elementary argument about the *object* suffices, and is no longer. The formal development, which has no notion of complete monotonicity at all, could not take the classical route and so found the elementary one; having found it, there was no reason to keep the classical route on paper. The net effect is that the results here rest on two cited facts rather than the six or seven a reader would expect, and that the one irreducible appeal to the derivative-sign definition — Lemma 7.1, where $sF'(s)$ is the object of study — is now visible as the single place where that vocabulary is doing work rather than providing convenience. We record this because the direction of influence is the opposite of the usual expectation: verification is generally described as a check on a finished argument, and here it shortened the argument.
