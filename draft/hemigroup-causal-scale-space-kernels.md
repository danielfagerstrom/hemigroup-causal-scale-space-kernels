...
Part (3) delineates exactly where the classical Sonine calculus with completely monotone kernel pairs — the standing hypothesis of most of the general-fractional-calculus literature — applies: on the complete-Bernstein subfamily, which the outlook (§10) identifies with the locality corner of the memory-line realization. For general admissible $F$ the pair, and Theorem 9.5, live at measure level.
...
| | Gamma (Ex. 8.2) | Bessel (Ex. 8.3) | stable ([F2005]) |
|---|---|---|---|
...
one $q$-step of scale per pole section. The limit kernel is thus the semi-selfdecomposable sibling of the Gamma family (Example 8.2), its Thorin measure the geometric atom train $\sum_{i} \delta_{q^{i}/c}$ in place of the single atom — placing it inside the subclass of Proposition 13.6, so that the Sonine-pair structure of §9 and the cascade implementation of §13 apply to it verbatim.

3. *(A8) in full: the Sato class of the present paper.*

Interpolation between the strata is cheap but not canonical: any ladder family extends to a continuous hemigroup in the first stratum by partial stages $\theta\log(1 + \mu s)$, $\theta \in [0,1]$, and the non-uniqueness of the extension is exactly the modulation freedom of $\mathfrak{F}$. This raises the selection question deferred to the companion note: whether the limit kernel is distinguished within its modulation orbit by an extremality principle (minimal modulation), which would, to our knowledge, be its first canonical characterization within an axiom class. The comparison with the previous literature is thereby one of containment rather than difference: the constructions of [Lindeberg–Fagerström 1996] and [Lindeberg 2016] are the first and second strata of the present axioms, and Theorem 7.3 is the third.
...
- Ph. Courrège, Sur la forme intégro-différentielle des opérateurs de $C_k^\infty$ dans $C$ satisfaisant au principe du maximum, *Séminaire Brelot–Choquet–Deny (Théorie du Potentiel)* **10** (1965/66), exp. 2.
- K.-J. Engel, R. Nagel, *One-Parameter Semigroups for Linear Evolution Equations*, Springer (2000).
- W. Feller, *An Introduction to Probability Theory and Its Applications*, Vol. 2, Wiley (1966/1971). [Ch. XIII: Laplace transforms, complete monotonicity, continuity theorem.]
- D. Fagerström, Temporal scale spaces, *Int. J. Computer Vision* (2005).
...
- R. Schilling, R. Song, Z. Vondraček, *Bernstein Functions: Theory and Applications*, 2nd ed., de Gruyter (2012). [Cited as SSV; Ch. 1–3, 6–7, 11.]
...
- R. Webster, Log-convex solutions to the functional equation $f(x+1) = g(x)f(x)$: $\Gamma$-type functions, *J. Math. Anal. Appl.* **209** (1997) 605–623. [Krull–Webster extension of Bohr–Mollerup.]
...
- D. V. Widder, *The Laplace Transform*, Princeton UP (1941). [Pringsheim–Landau singularity theorem for transforms with nonnegative integrand.]
