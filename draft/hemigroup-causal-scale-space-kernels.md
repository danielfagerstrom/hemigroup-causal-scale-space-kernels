...
Both clauses are conditions at the zero-delay end of the kernels. The first says exactly that they put no atom at zero delay — equivalently, $b_0 > 0$ or $\int_0^\infty k(t)\,t^{-1}dt = \infty$ — and it is automatic for every admissible $F \not\equiv 0$, so all of (H)'s bite is in the second; it is stated nonetheless, being what the proofs consume. The second implies $\int_0^\infty e^{-F(s)}\,ds = \mathbb{E}[T_1^{-1}] < \infty$, and the converse fails only at the endpoint: finiteness forces $z_* \ge 1$, not $z_* > 1$, and the endpoint is genuinely reached — the Gamma family at $\gamma = 1$ has $z_* = 1$ with $\mathbb{E}[T_1^{-1}] = \infty$. The second clause holds with $z_* = \infty$ whenever $b_0 > 0$, for the stable family, and for the Bessel-K family of Example 8.3; for the Gamma family $z_* = \gamma$, so (H) requires $\gamma > 1$ there. (The construction extends below $z_* = 1$ by meromorphic continuation; we do not pursue this.) The threshold $1$ is the one this section keeps meeting — see the proofs of Lemma 11.5 and Theorem 11.6(2), where it arises three separate times — and by Remark 7.6 it is exactly the abscissa of the extreme rays of the admissible cone: (H) excludes the cone's extreme boundary and nothing else.
