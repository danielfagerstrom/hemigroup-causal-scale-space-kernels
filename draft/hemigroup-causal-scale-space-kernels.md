...
Everything §§4–6 contributes to Theorem 7.3 is verified with it: the convolution representation, the additivity and infinite divisibility of the exponents (Theorem 5.2), and the similarity form forced by covariance. In §9, the memory kernel, its transform, the Sonine conservation identity $\kappa^{(x)} * \ell^{(x)} = \mathrm{Leb}$ and the existence and uniqueness of the potential kernel are verified. Of §11, all fourteen numbered items are.
...
**What is not formalized, and why.** §10 (Theorem 3′, the scale-Cauchy problem) and §12 (Theorem 5′, the locality theorem) are not, for a reason external to this paper: they need the theory of $C_0$-semigroups and closed operators, and the Bessel functions $K_\nu$, none of which Mathlib carries yet. Two results in §9 — the distributional scale-evolution identity (Proposition 9.2) and the exact-inversion corollary — need distributions paired with measures, likewise absent. Proposition 9.7(3), the characterization of when $\kappa^{(x)}$ has a completely monotone density, is a statement *about* the CM class, which the formal development deliberately does not define; it is cited, not verified. §§8 and 13–14 are examples and implementation, formalized only where they were cheap.
