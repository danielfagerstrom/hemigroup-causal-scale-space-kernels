...
\begin{scope}
  \node[anchor=west] at (-0.5,2.45) {(a) catalogues are profiles over log-delay; dilation is translation};
  % axes
  \draw[->] (-0.5,0) -- (5.6,0) node[anchor=north] {$\theta = \log t$};
  \draw[->] (-0.5,0) -- (-0.5,2.2) node[anchor=south] {$\tilde k(\theta)$};
  ...
  \node[anchor=west] at (2.55,0.42) {stable $c\,e^{-\alpha\theta}$: scale-free, heavy tails};
  ...
  \node[anchor=west] at (-0.35,1.66) {Gamma: plateau, then cliff};
  % Dickman ray: unit step to cutoff log tau
  \draw[thick, densely dotted] (-0.5,0.85) -- (2.2,0.85) -- (2.2,0.02);
  \node[anchor=south west] at (-0.35,0.88) {Dickman ray $\mathbf{1}_{\theta < \log\tau}$};
  \draw (2.2,0.07) -- (2.2,-0.07); \node[anchor=north] at (2.2,-0.1) {$\log\tau$};
  % dilation = translation
  \draw[->] (3.6,1.85) -- node[above] {$\dil_\sigma$: shift by $\log\sigma$} (5.0,1.85);
\end{scope}

% ---------- panel (b): the Dickman superposition ----------
\begin{scope}[yshift=-3.9cm]
  \node[anchor=west] at (-0.5,2.45) {(b) every admissible profile is a unique stack of steps};
