...
## Verdict (P6, 2026-08-15)

**The Lean proves what the article states.** Sixty-six `\leanok` nodes; every one audited by a
card (Tier 0: 9 definition cards; Tier 1: the two headline theorems, the nine-node supply chain,
the two ledger axioms, Cor. 7.4; Tier 2: 18 cards over chapters 9, 10, 12; Tier 3: 16 cards over
chapters 8 and 2), each unfolded to Mathlib primitives, junk-value audited, and — for the
definitions and the headline theorems — compared with blind restatements written from the draft
alone and attacked by adversarial-vacuity passes. **No conjunct of any proved theorem was found
vacuous, junk-valued, or weaker than the article's proof establishes**, and no hypothesis was
found stronger than the article's beyond representative-level choices (pointwise causality of a
representative, the `k 0 = 0` normalisation, `Measurable` on functions the article takes as `L¹`
classes). The two ledger axioms are, row by row, no stronger than their image-verified sources.

**What the review changed.** Twenty-seven findings; none claim-changing. Eleven were
*statement-tightenings* — proved content that the assembled theorem did not state — and all are
now conjuncts: Theorem 2′'s (⇒) concludes in `SelfDecomposableExponent` with `χ 1 = 1` and
identifies the kernels (`main_analysis'`), (⇐) holds in an arbitrary gauge
(`CascadeFamily.reparam`), Theorem 4′ carries its domain claim, `u = Φ_{0,x} f`, the boundary
value, the `X₀`-derivative and the Mellin convergence, Sonine conservation is an unrestricted
equality of measures, Lemma 6.1 has both directions. Two were *article-side* corrections, where
the article's statement had outrun its own proof: Thm 11.6(2)'s Mellin form is stated off the
zeros of `H̃(z−1)`, and Lemma 11.4 asks for the eigen-relation at every height (its proof had
jumped from one line to the strip). The rest were notes — a stale annotation, an unpackaged
elementary converse, a tag naming one of three declarations, an interlocking of anchors —
resolved in the text of record or accepted with a sentence. The draft and the blueprint were
diffed statement by statement in every chapter and are aligned; the proof routes were swept and
diverge only in the recorded, deliberate ways.

**Witnesses.** `Witnesses.lean` shows the headline hypotheses jointly satisfiable at pure drift,
Gamma (`γ > 1`) and stable, the pure-delay `delayCore` doing so in Lean core; nothing needed a
restriction the article does not state.
...
| R28 | F6 | retirement | ledger A17, `exists_isFiniteMeasure_laplace_eq_exp_neg_levyExponent` (`Interfaces.lean`) | A17 was an admitted axiom; the compound-Poisson construction that retires it had been named but not carried out. | **resolved (Q-0022, 2026-09-21)** — now a `theorem`, statement unchanged character for character, proved in `CompoundPoisson.lean` (compound-Poisson laws of the truncations, tightness, Prokhorov cluster point, drift by translation); removed from `trust-boundary.txt`; `CIAxiomGuard.lean` re-run, every former A17 line now Lean core (or Lean core + A18). No downstream statement touched. |
| R29 | F3 | statement-tightening (Lean side) | `lem:criterion-converse`, `SelfDecomposableExponent.integrableOn_of_ne_top` (`ExponentDerivative.lean`) | The node's content is that finiteness of (7.1) at the single point `s = 1` forces both integrals; the tagged declaration took a whole `SelfDecomposableExponent` (finiteness at every `s ≥ 0`, and `k 0 = 0`). The proof used only `s = 1`, but the type did not show it. Q-0138 audit, hypothesis drift. | **resolved (Q-0141), Lean strengthened** — `integrableOn_of_levyExponentD_one_ne_top` over a bare `(b₀, k)`: `k ≥ 0`, `AntitoneOn k (Ioi 0)`, `levyExponentD b₀ k 1 ≠ ⊤` (no sign on `b₀`); the structure's form is now its instance; node's tag moved, statement unchanged; guarded, Lean core |
| R30 | F3 | statement-tightening | `lem:log-convexity`, `convexOn_log_negMoment` (`Locality.lean`) | Node: "`m` is log-convex on `(0,∞)`", no hypothesis. Tagged declaration: `ConvexOn` of `log (negMoment ·).toReal` on `momentInterval = (0, z_*)`, under `lawT₁ {0} = 0`. The unconditional `[0,∞]` Hölder form `negMoment_le_rpow_mul_rpow` covers the node's first reading but was not in the tag. Q-0138 audit, hypothesis drift. | **resolved (Q-0141), node corrected** — `% CHANGED`: the statement now gives the `[0,∞]` Hölder inequality on `(0,∞)` (no hypothesis) and the real-valued convexity of `log m` on `(0, z_*)` under no atom at `0`; tag lists both declarations. The no-atom half of the drift was ambient (chapter 12 assumes (H) throughout; `lawT₁_singleton_zero`); the domain half was real. The paper's transcription (`paper/10-locality.tex`) keeps the one-line form: a `v1.1` item in `notes/HANDOFF.md` |
| R31 | F3 | statement-tightening (both sides) | `lem:inversion-operator-action`, `inversionOperator_eq` (`InversionOperator.lean`) | Node: "Fix `c > 0`", product identity a.e. on the line, no (H). Tagged declaration: `StandingHypothesis`, `c < z_*`, and the identity off the zeros of `H̃` (`RealisesSymbolAction`); (H) and the strip serve only to make those zeros null. The strip is partly ambient (`def:inversion-operator` fixes `c ∈ (0, z_*−1)`), (H) is not. Also found: the second display `Ãg(z) = h̃(z−1)` needs `h` continuous on `(0,∞)` (`ContinuousOn h (Ioi 0)`), which the node did not state. Q-0138 audit, hypothesis drift. | **resolved (Q-0141), Lean strengthened, node corrected in one clause** — `inversionOperator_eq_of_ae`, `mellin_inversionOperator_of_ae`, `mellin_inversionOperator_eq_of_ae`: hypotheses are the node's (a.e. identity, `MellinConvergent`, `VerticalIntegrable`), no (H), no bound on `c`; the `RealisesSymbolAction` forms are now their instances. `% CHANGED`: "if `h` is continuous on `(0,∞)`" added to the second display. The paper's transcription (`paper/09-signaling.tex`) lacks it: a `v1.1` item in `notes/HANDOFF.md` |
