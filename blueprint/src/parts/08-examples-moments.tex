...
% (Halgreen) and \ref{prop:moments} on A7 (Sato). Both say more than their citation carries,
...
\uses{lem:selfdecomposable-exponents,cor:semigroup-case,prop:moments}
...
are infinite by \ref{prop:moments}. The mode scaling is the self-similarity
...
\uses{lem:selfdecomposable-exponents,thm:main-characterization,prop:moments}
...
the subject of the memory-line development, Theorems 10.4, 11.5 and 12.5.
...
moment criterion is \ref{prop:moments} applied to this $k$. The parabolic-gauge reading is a
...
$\int_1^\infty t^{n-1}k(t)\,dt < \infty$ exactly when $n < a$; \ref{prop:moments} converts
that into $\Ex\,T_x^n < \infty$ iff $a > n$.
\end{proof}

% draft: Proposition 8.4
\begin{proposition}[moments and the influence curve]\label{prop:moments}
...
is integrable at infinity; more generally $\Ex\,T_x^n < \infty$ if and only if
$\int_1^\infty t^{\,n-1} k(t)\,dt < \infty$. When the mean is finite, the influence curve of
...
\textbf{Assignment.} \ledger{A7} carries the higher-moment equivalence alone --- that a moment
of the law is finite exactly when the corresponding integral against the Lévy measure over
$\{t > 1\}$ is finite. The first-moment identity $\Ex\,T_x = xF'(\zp)$, the evaluation
$F'(\zp) = b_0 + \int k$, and the exact linearity of the influence curve are \textbf{[T]} and
proved below from (7.1) by monotone convergence; they need no citation. What is \emph{not}
claimed here is anything about the mode, which is the delay measure the stable family forces
one to use --- see \ref{prop:stable-family}.)}
\end{proposition}
...
infinity. For the higher moments, the Lévy measure of $T_x$ is $\nu(dt) = k(t)t^{-1}dt$ up to
the scaling by $x$, so $\int_{t>1} t^n\,\nu(dt) = \int_1^\infty t^{\,n-1}k(t)\,dt$; \ledger{A7}
equates finiteness of that integral with finiteness of $\Ex\,T_x^n$. The influence curve is
then linear in $x$ because $\Ex\,T_x$ is.
\end{proof}

% draft: Remark 8.5
\begin{remark}[heavy tails are the semigroup's doing]\label{rem:heavy-tails}
\uses{thm:main-characterization,cor:semigroup-case,prop:moments,prop:gamma-family}
