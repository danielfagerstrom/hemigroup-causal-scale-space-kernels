...
$\sigma$, then $x^* = 0$.
\end{enumerate}
\statusT\quad\emph{(\textbf{Not formalised.} One note for the Lean pass: the proof of (3) below
argues that a nonzero member of $\BFz$ is strictly increasing from the sign of its derivative,
which is vocabulary the development does not have. The substitute is
\ref{cor:strict-monotonicity}, which reaches the same conclusion by running \ref{lem:vanishing}
on the representation and never differentiating; the constructed-family case is already in Lean
as \texttt{exponent\_strictMono}. Nothing else in this lemma leaves the order structure, so
(1), (2) and (4) are Mathlib's order API and an injectivity argument.)}
\end{lemma}
...
carried along, not re-established. \textbf{Not formalised}, and this construction is the
unglamorous stretch the formalisation note has always flagged (M5 of the ladder). Beyond the
gauge itself, note what the Lean will need that the statement does not show: $\chi$ bundled with
its inverse and with the conjugation $\chi \circ \Sact_\sigma = \sigma\,\chi$, which is the
interface \ref{prop:main-uniqueness} deliberately declined to guess at. Design it here, against
this proposition, and \ref{thm:main-analysis} is assembly.)}
\end{proposition}
