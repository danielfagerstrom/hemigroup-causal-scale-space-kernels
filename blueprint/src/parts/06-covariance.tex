...
$\sigma$, then $x^* = 0$.
\end{enumerate}
\statusT\quad\emph{(\textbf{Three clauses of four are formalised.} (1) is
\texttt{eq\_of\_G\_eq}: it \emph{is} injectivity of $G(\cdot,s)$, which is
\ref{cor:strict-monotonicity}. (2) is \texttt{S\_comp} and \texttt{S\_one}, two applications of
(6.1) and the same injectivity. (4) is \texttt{eq\_zero\_of\_fixed}, where the only analysis is
that $G(x,\cdot)$ is continuous at the origin with value $0$ --- which turns ``constant on
$(0,\infty)$'' into ``identically zero'' and so contradicts
\ref{cor:strict-monotonicity} above $x = 0$.}

\emph{What (3) needed, and did not have, was strict monotonicity in the \emph{other} variable:
the proof below reads it off the sign of a Bernstein function's derivative, which is vocabulary
the development does not carry. The substitute is \texttt{laplace\_strictAnti}, one line of
measure theory --- the support of $e^{-st} - e^{-s't}$ is exactly $\{t \ne 0\}$, and that set
carries positive mass precisely because the kernel is not $\delta_0$, which is (ND). No
derivative and no Bernstein class appears. What remains of (3) is its continuity half, and the
shape it will take is the blueprint's own: the inverse of $G(\cdot,s_0)$ on its range, composed
with $\sigma \mapsto G(x,\sigma s_0)$. The Lean tag names the four-clause target type, stated
and \texttt{sorry}-marked in \texttt{Formalization/Skeleton/}.)}
\end{lemma}
...
\lean{Skeleton.exists_canonical_gauge}\notready
...
carried along, not re-established. \textbf{Not yet proved}, and this construction is the
unglamorous stretch the formalisation note has always flagged (M5 of the ladder). Its target
type is written, though, and that is where the design decision the note warned about was
taken: \texttt{Skeleton.exists\_canonical\_gauge} asserts $\chi$ together with the conjugation
$\chi(\Sact_\sigma x) = \sigma\,\chi(x)$ and the similarity form
$G(x,s) = G(1, \chi(x)\,s)$, rather than $\chi$ alone. That bundle is the interface
\ref{prop:main-uniqueness} deliberately declined to guess at, and fixing it here is what makes
\ref{thm:main-analysis} assembly.)}
\end{proposition}
