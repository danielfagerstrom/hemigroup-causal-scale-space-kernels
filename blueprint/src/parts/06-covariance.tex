...
% Draft §6; renders as Chapter 6 since the 2026-09-01 reorder to the article's order
% (content.tex has the rationale). Historical comments below that cite chapter or node
% numbers ("Numbered 6.M") use the retired draft-aligned numbering; the `% draft:`
...
\uses{def:cascade-family,lem:convolution-representation,prop:laplace-uniqueness}
...
\]
\statusT\quad\emph{(1)$\Rightarrow$(2)$\wedge$(3) is \texttt{covariance\_laplace}, and
(2)$\Rightarrow$(1) is \texttt{isScaleCovariant\_of\_repr\_map} (\texttt{Covariance.lean},
fidelity review R18); (2)$\Leftrightarrow$(3) needs no further declaration, being the
transform identity $\widehat{\dil_\sigma\mu}(s) = \hat\mu(\sigma s)$ together with
injectivity of the Laplace transform on causal probability measures and of $-\log$ on its
positive values, both already in play in the proof below.}
\end{lemma}
...
that an identity of measures; this is (1)$\Leftrightarrow$(2). For (2)$\Leftrightarrow$(3),
...
\ref{lem:convolution-representation}.
\end{proof}
...
\ref{prop:canonical-gauge} fixes the \emph{canonical gauge} $\tilde x := \chi(x)$,
...
recurs, for the Bessel family of Chapters~8--10: the
