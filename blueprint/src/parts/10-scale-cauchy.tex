...
\]
\statusT\quad\emph{(Stated in Lean, unproved. The setting this lemma is about --- $\Xz$, the
delay semigroup and $\core$ --- is defined and its elementary theory proved in
\texttt{Hemigroup/DelayCore.lean}; the five clauses are stated in
\texttt{Skeleton/Chapter10.lean}. $\Xz$ is modelled as a predicate on $X = L^1(\RR)$, not as a
subtype, on the same grounds as the measure side's \texttt{IsCausal}; and $\core$ is modelled
as a predicate on genuine functions, because Chapters 11--12 quantify over its consequences
\emph{pointwise} and an $L^1$ class has no pointwise values. That $\core$ so defined supplies
exactly the hypotheses \ref{thm:signaling-form} takes about its signal --- neither fewer nor
more --- is itself a machine-checked equivalence.)}
\end{lemma}
