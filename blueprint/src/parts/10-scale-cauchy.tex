...
\]
\statusT\quad\emph{(\textbf{Not blocked by generation theory}, re-checked 2026-08-14. Everything
this definition names now exists: $\core$ and $(T_r)$ are \ref{lem:delay-core}'s, built in
\texttt{Hemigroup/DelayCore.lean}, and $\nu_1$ is chapter~9's \texttt{exists\_tailMeasure} --- the
quantile construction made for \ref{lem:potential-kernel} precisely because
\texttt{StieltjesFunction} does not apply to a L\'evy tail. The integral is an $\Xz$-valued
Bochner integral, which needs no new theory: $X = L^1(\RR)$ is a complete normed real space, $T_r$
is a continuous linear map on it, and $r \mapsto T_rf$ is continuous, so the integrand is strongly
measurable and the definition is total.
...
\end{enumerate}
\statusT\quad\emph{(\textbf{Not blocked by generation theory}, re-checked 2026-08-14: no clause
mentions a generator's domain, a resolvent, or a generation theorem, and the ``strong continuity
of $T$'' that (4) invokes is \texttt{continuous\_transL1}, proved in chapter~4. Clause by clause
