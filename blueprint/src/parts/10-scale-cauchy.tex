...
\end{enumerate}
\statusT\quad\emph{(\textbf{Stated in Lean, clauses (1), (3), (4) proved} (2026-08-14):
\texttt{Skeleton/Chapter10.lean} carries the two open clauses as named sub-lemmas with a
\texttt{sorry}-free collation above them, which is the decomposition gate's middle state and not a
claim; the three proved clauses have moved into \texttt{Hemigroup/PhillipsGenerator.lean} and are
...
measures.
What (2) does cost is worth naming in advance: $\Lap$ is not a bounded functional on $L^1(\RR)$,
$e^{-st}$ being unbounded to the left of the origin, so the exchange is done on causal
representatives in $[0,\infty]$ --- as in chapter~2's Tonelli identity and in
\ref{lem:delay-core}'s estimate --- rather than by pulling a continuous linear map through the
Bochner integral, which is available only for (3).)}
\end{lemma}
