...
ingredient. Convergence of the \emph{increments} and hence of the
transforms follows (\texttt{tendsto\_laplace\_kernel}), carried out in $\RR$ rather than
$[0,\infty]$ because truncated subtraction is not continuous. What is left is the assembly
alone: Prokhorov for a convergent subsequence, portmanteau for causality of the limit, and
\ref{prop:laplace-uniqueness} to identify it. That needs no citation, but it is not written, so
\ledger{A5} stays a live candidate for the trust boundary --- the evidence says avoidable, not
avoided.)}
\end{proposition}
