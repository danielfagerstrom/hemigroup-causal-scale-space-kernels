...
$T$ large yields tightness with no cited input. That does \emph{not} yet make \ledger{A5}
avoidable: turning tightness into (A7) still needs pointwise convergence of the transforms, a
dominated-convergence argument on the Lévy integral, and then Prokhorov together with
\ref{prop:laplace-uniqueness} to identify the limit. Both are within reach and neither needs a
citation, but neither is written.)}
\end{proposition}
