...
% Draft §2; renders as Chapter 3 since the 2026-09-01 reorder to the article's order
% (content.tex has the rationale). Historical comments below that cite chapter or node
% numbers ("Numbered 2.M") use the retired draft-aligned numbering; the `% draft:`
...
\emph{Which vocabulary does the work.} \ref{prop:bernstein-toolbox} is used as a bridge and not as
a toolkit. The object the arguments manipulate is the \emph{representation}, part~(3) --- a drift
and a Lévy measure --- and almost every proof in Chapters~4--9 and the appendix works with it directly: the
increments of \ref{thm:increments-bernstein} are produced as representations, not as pointwise
limits of Bernstein functions; the potential kernel of \ref{lem:potential-kernel} is constructed as
a subordinator's potential measure, not represented from complete monotonicity; the Mellin calculus
of Chapter~9 mentions neither class. $\CM$ and the derivative-sign definition of $\BF$ are needed
at exactly three places, and it is worth naming them: \ref{lem:selfdecomposable-derivative}, where
$B(s) = sF'(s)$ has no meaning until the representation is in hand; \ref{prop:pair-regularity}(2),
together with the draft's Thorin-subclass proposition (\S13, not transcribed here), where $\CM$ is
the \emph{subject} rather than a tool; and
\ref{prop:bernstein-toolbox} itself, which identifies the two vocabularies once so that no later
proof has to. A reader who wants the shortest path to \ref{thm:main-characterization} or
\ref{thm:signaling-form} may take (3) as the definition of admissibility and never invoke
Bernstein--Widder. \emph{(This is also the development's design: it defines admissibility by the
representation and has no $\CM$ at all --- see \texttt{DESIGN-formalization-strategy.md}. The
paragraph records that the choice is available on paper too, and what it costs, which is nothing
outside those three places.)}
...
\emph{\textbf{Extended 2026-08-11 to measures that are not finite}, which is what the appendix
...
% response R6): the uniqueness propositions are positive-measure statements, while four
% proofs -- prop:scale-evolution, lem:generator-properties(5), thm:scale-cauchy(3),
% cor:signaling-wellposed -- apply uniqueness to differences of causal functions. This
% corollary records the signed case by Jordan decomposition; the mirrored proofs in this
% blueprint keep their positive-measure shorthand, and this node is what discharges it.
...
\statusT\quad\emph{(The Jordan-decomposition reduction to
\ref{prop:laplace-uniqueness-sigma-finite}, and nothing more; recorded so that the signed
applications named in the header comment rest on a stated node rather than on shorthand.)}
\end{corollary}
...
\ref{prop:laplace-uniqueness-sigma-finite} the two measures coincide, so
...
\emph{The consequential sentence, for an arbitrary family rather than only the constructed
kernels}, is \texttt{Hemigroup.exists\_tail\_le\_of\_forall\_laplace} (the tail bound, in the
``for every $\eta$'' form \texttt{tendsto\_integral\_of\_tendsto\_laplace} consumes) and
\texttt{Hemigroup.isTightMeasureSet\_of\_forall\_laplace} (the same, in Mathlib's
\texttt{IsTightMeasureSet} vocabulary) --- same $s$-then-$T$ argument, read off the hypothesis
\texttt{$\forall \varepsilon > 0,\, \exists s > 0,\, \forall i,\, 1 - \hat\mu_i(s) \le
\varepsilon$} directly instead of through $F(Bs)$. Both reduce to Lean core; the kernel-family
lines above are not re-derived from them, since doing so would just restate
\texttt{exists\_kernel\_tail\_le}'s own proof of the hypothesis rather than shorten it.)}
\end{lemma}
