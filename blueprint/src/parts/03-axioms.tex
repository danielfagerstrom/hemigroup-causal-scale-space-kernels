...
\end{itemize}
\statusT\quad\emph{(Stated in Lean as the structure \texttt{Hemigroup.CascadeFamily}, one field
per clause, with (A1) carried by the type of bounded linear operators on $X$ and (A8)'s
$\Sact$ bundled as data. This is the one file in the development where an error would be
silent --- a mis-stated axiom makes the theorems vacuous rather than unprovable --- so the
specification is checked against a model: \texttt{Hemigroup.cascadeFamily} exhibits the kernels
of \ref{thm:main-characterization} as an instance. Doing so caught two defects in an earlier
