...
% on trust.
%
% Banach--Alaoglu and Riesz--Markov (the weak-* extraction) are likewise held [T], and are
% deliberately not ledger entries.
%
% CORRECTION (2026-08-09, from scouting M6b). The original note here said "Mathlib has both",
% which is only half right, and the half that is wrong is the one that matters:
%
...
%     Λ : C_c(X, ℝ≥0) →ₗ[ℝ≥0] ℝ≥0. There is no M(X) ≅ C₀(X)* duality anywhere in Mathlib.
%
% The conclusion survives, because the functionals here ARE positive (h_ε ≥ 0), so the duality
% is not actually needed: what is needed is the extraction of a limit functional, and then
% rieszMeasure applies. But the step is hand-work, not a citation, and the cost estimate for
% M6b has to reflect that. See the note on the alternative route below.
...
Riesz--Markov, and the functionals in play are \emph{positive}, which is the form in which the
representation theorem is available.

\emph{Note for the formalisation.} The extraction admits a second route, and it may be the
cheaper one. The identity $\Phi(f * \rho_\varepsilon) = f * h_\varepsilon$ used in the proof
gives $f * h_\varepsilon \to \Phi f$ in $X$ for \emph{every} $f$, and convergence in $X$ of the
densities makes the family $\{h_\varepsilon\,dt\}$ tight, so a subsequential weak limit is
available from Prokhorov directly, as a probability measure, with no dual space in sight. The
mass and support clauses then come for free rather than needing $\|\mu\| = 1$ to be recovered
afterwards.
