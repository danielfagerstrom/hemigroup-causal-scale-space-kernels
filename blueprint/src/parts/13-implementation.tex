...
% Draft §13; renders as Chapter 11 since the 2026-09-01 reorder to the article's order
% (content.tex has the rationale). Historical comments below that cite chapter or node
% numbers ("Numbered 13.M") use the retired draft-aligned numbering; the `% draft:`
...
% nodes below are blueprint statements first, and none carries a \lean tag or is scheduled in
% PLAN-chapters-8-12.md.
%
% UNSCHEDULED, NOT EXCLUDED (hub ADR-0020, 2026-09-22): every blueprint node is a
% formalisation target in the end. What to prove next is a cost-benefit call within the
% programme's order: a fact Mathlib lacks is admitted as a hypothesis (a ledger entry) rather
% than built, unless building it is decided; a node with many dependents ranks above a leaf;
% results an implementer relies on rank up, since a subtle error in an algorithm is paid for by
% whoever builds from the paper; the size is priced in Lean lines. This article's unscheduled nodes are
% priced in a report before any of them is queued; the programme's order puts Paper V first.
%
